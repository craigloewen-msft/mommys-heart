...
/// Show Funding only when both the admin role and information grant allow it.
pub fn require_information_management_admin<F>(state: AppState, content: F) -> AnyView
where
    F: Fn() -> AnyView + Send + 'static,
{
    (move || {
        if state.is_authenticated() {
            if state.has_information_management_admin_access() {
                content()
            } else {
                view! { <Redirect path="/cases" /> }.into_any()
            }
