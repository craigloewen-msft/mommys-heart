...
mod tests {
    ...
    fn slugify_matches_python_rules() {
        ...
        assert_eq!(slugify("  Family Court Guide  "), "family-court-guide");
        assert_eq!(slugify("Benefits & Resources!"), "benefits-resources");
        assert_eq!(slugify("A__B  C"), "a-b-c");
    }

    #[test]
    fn chunking_respects_size_and_overlap() {
        // Build sections whose combined size exceeds CHUNK_SIZE so we get >1 chunk.
        let big = "x".repeat(4 * 300); // ~300 tokens each
        let sections = vec![
            Section {
                text: big.clone(),
                heading: "H1".into(),
        ...
        assert!(chunks.iter().all(|c| c.filename == "f.docx"));
    }

    #[test]
    fn extracts_real_corpus_if_present() {
        let dir = std::path::Path::new("docs");
        if !dir.exists() {
            return; // corpus not present in this environment; skip.
        }
        let mut total = 0;
        for entry in std::fs::read_dir(dir).unwrap().flatten() {
            ...
            if path.extension().and_then(|e| e.to_str()) == Some("docx") {
                let sections =
                    extract_sections(&path).unwrap_or_else(|e| panic!("extract {path:?}: {e}"));
                assert!(!sections.is_empty(), "no sections from {path:?}");
                let chunks = chunk_sections(&sections);
                assert!(!chunks.is_empty(), "no chunks from {path:?}");
                total += chunks.len();
            }
        }
        assert!(total > 0, "expected chunks from the docs corpus");
    }
}
