...
mod tests {
    ...
    fn ctx(filename: &str, heading: &str) -> store::Retrieved {
        store::Retrieved {
    ...
    #[test]
    fn cited_documents_stay_documents() {
        let contexts = vec![ctx("a.docx", "Benefits")];
        let raw = r#"{"answer":"A","used_sources":[1],"source_type":"documents"}"#;
        let resp = build_response(raw, &contexts);
        assert_eq!(resp.source_type, SourceType::Documents);
        assert_eq!(resp.sources.len(), 1);
        assert_eq!(resp.sources[0].anchor, "benefits");
        assert_eq!(resp.sources[0].relevance, 0.812);
    }

    #[test]
    fn documents_without_citations_downgrade_to_general() {
        let contexts = vec![ctx("a.docx", "Benefits")];
        let raw = r#"{"answer":"A","used_sources":[],"source_type":"documents"}"#;
        let resp = build_response(raw, &contexts);
    ...
    #[test]
    fn general_knowledge_with_citations_becomes_mixed() {
        let contexts = vec![ctx("a.docx", "Benefits")];
        let raw = r#"{"answer":"A","used_sources":[1],"source_type":"general_knowledge"}"#;
        let resp = build_response(raw, &contexts);
        assert_eq!(resp.source_type, SourceType::Mixed);
    }

    #[test]
    fn sources_are_deduplicated_and_indices_bounds_checked() {
        let contexts = vec![ctx("a.docx", "Benefits"), ctx("a.docx", "Benefits")];
        // Index 3 is out of range and must be ignored; the two identical
        // (filename, heading) sources collapse to one.
        let raw = r#"{"answer":"A","used_sources":[1,2,3],"source_type":"documents"}"#;
        let resp = build_response(raw, &contexts);
        assert_eq!(resp.sources.len(), 1);
    }

    #[test]
    fn non_json_response_is_treated_as_plain_answer() {
        let contexts = vec![ctx("a.docx", "Benefits")];
        let resp = build_response("just a plain answer", &contexts);
        assert_eq!(resp.answer, "just a plain answer");
        assert_eq!(resp.source_type, SourceType::GeneralKnowledge);
    }
}
