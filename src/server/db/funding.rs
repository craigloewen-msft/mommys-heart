...
const SELECT_COLUMNS: &str = "f.id, f.kind, f.amount_cents,
     ...
     f.reference, f.notes, f.voided, f.void_reason, f.recorded_by,
...
pub async fn create(input: &ValidatedFunding, actor: &str) -> Result<String, sqlx::Error> {
    let id = ids::next(pool(), "fn").await?;
    let mut tx = pool().begin().await?;
    sqlx::query(
        "INSERT INTO funding
             (id, kind, amount_cents, received_on, grant_id, source_organization_id,
              source_contact_id, reference, notes, recorded_by)
    ...
    // Audited against the grant when there is one, so a grant's Change Log shows
    // its own money coming in.
...
pub async fn void(id: &str, reason: &str, actor: &str) -> Result<(), sqlx::Error> {
    let mut tx = pool().begin().await?;
    ...
    sqlx::query(
        "UPDATE funding
         SET voided = true, void_reason = $2, voided_by = $3, voided_at = now()
         WHERE id = $1",
    )
    .bind(id)
    .bind(reason)
