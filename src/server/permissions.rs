//! Authorization (permission checks) for the server-function layer (SSR only).
//!
...
//! [`require_user`] (from the session cookie) and then checks permissions with
//! the helpers here. Keeping access control in one place — rather than repeating
//! it in each operation — means every authenticated entry point is guarded the
//! same way before it ever touches the database in [`crate::server::db`].
//!
