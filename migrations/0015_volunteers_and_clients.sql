...
-- place rather than filing a second record. A denied applicant may accept the
-- agreement again, which overwrites the row back to 'pending'.
...
-- Backfill from the role already on `users`. Existing volunteers are approved
-- with an empty `agreement_version`: they are volunteers, but they never saw the
-- agreement, which is exactly what an empty version records.
